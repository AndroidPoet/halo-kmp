...
/// app's bridge glue forwards to `HaloKit.shared` from any thread.
///
...
/// `end(id:...)`).
@available(iOS 16.2, *)
public final class HaloKit {
    public static let shared = HaloKit()

    private let lock = NSLock()
    private var activities: [String: Activity<HaloActivityAttributes>] = [:]
    ...
    public func update(id: String, contentJson: String, staleAtEpochMillis: Int64, completion: @escaping (String?, String?) -> Void) {
        ...
        guard let state = try? decoder.decode(HaloActivityAttributes.ContentState.self, from: Data(contentJson.utf8)) else {
    ...
    public func end(
        ...
        let finalState = finalContentJson.flatMap { json in
            try? decoder.decode(HaloActivityAttributes.ContentState.self, from: Data(json.utf8))
        }
    ...
    private func track(_ activity: Activity<HaloActivityAttributes>) {
...
struct ActivityRecord: Codable {
    ...
    var content: HaloActivityAttributes.ContentState
}
